...
class Class_Form_Block {
	...
	public function __construct() {
		// Всё регистрируем строго внутри одного хука init
		add_action( 'init', [ $this, 'register_gutenberg_block' ] );
	}

	public function register_gutenberg_block() {
		// Исправляем двойной слеш: убираем первый слеш перед assets, так как AFB_URL уже со слешем
		wp_register_script(
		...
		// Регистрируем блок и ОФИЦИАЛЬНО привязываем скрипт через параметры WP
		register_block_type( 'afb/form-block', [
			'editor_script'   => 'afb-form-block-editor', // WP сам разберется, когда и на какой хук его вставить
